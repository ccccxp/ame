...
:: Check and install mod-tools (skip if already installed)
echo [1/3] Checking mod-tools...
if exist "%TOOLS_DIR%\mod-tools.exe" (
    echo   mod-tools already installed, skipping.
) else (
    echo   Installing mod-tools to: %TOOLS_DIR%
...
:: Check and install Pengu Loader (skip if already installed)
echo.
echo [2/3] Checking Pengu Loader...
if exist "%PENGU_DIR%\Pengu Loader.exe" (
    echo   Pengu Loader already installed, skipping.
) else (
    echo   Installing Pengu Loader to: %PENGU_DIR%
...
:: Always copy ame plugin to Pengu plugins folder
echo.
echo [3/3] Installing ame plugin...
...
echo.
if "%FAILED%"=="1" (
    echo Some components failed to install. Check your internet connection.
) else (
